...
//MaximWire::DS18B20 sondeTemperatureIn3("2894FCC90B00009F");
...
void getTemperatureAndHumidityOut()
{
  ...
  if (!isnan(localTemperatureOut))
  {
    ...
    oled.clearDisplay();
    oled.setTextXY(0,0);
    oled.putString("Temp. ext. :");
    oled.setTextXY(1,3);
    oled.putString(String(temperatureOut));
    */
  }
  else
  ...
  if (!isnan(localHumidityOut))
  {
    ...
    oled.clearDisplay();
    oled.setTextXY(0,0);
    oled.putString("Humid. ext. :");
    oled.setTextXY(1,3);
    oled.putString(String(humidityIn));
    */
  }
  else
...
void getBatteryState()
{
  ...
  /*
  int rawBatteryState = analogRead(batteryStatePin);
  Serial.println(rawBatteryState);
  int localBatteryState = (rawBatteryState - 790)*100/170;
  localBatteryState = max(min(localBatteryState, 100), 0);
  Serial.print(localBatteryState);
  Serial.println(" <== Etat de la batterie : success !");
  batteryState = localBatteryState;
  */
  ...
  oled.clearDisplay();
  oled.setTextXY(0,0);
  oled.putString("Batterie :");
  oled.setTextXY(2,3);
  oled.putString(String(batteryState));
  */
}
...
void getLuminosity()
{
  ...
  oled.clearDisplay();
  oled.setTextXY(0,0);
  oled.putString("Luminosite :");
  oled.setTextXY(2,3);
  oled.putString(String(luminosity));
  */
}
...
void sendPayload()
{
  ...
  Serial1.print("AT$SF=");
  for (int i = 0; i < 12; ++i) {
    Serial1.print(payloadArray[i], HEX); 
  }
  Serial1.println("");
  */
}
